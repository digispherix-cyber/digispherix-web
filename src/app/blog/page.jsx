    ...
    <main style={{ minHeight: '100vh', color: 'var(--text)' }}>
      ...
      <section style={{ paddingTop: '140px', paddingBottom: '60px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="ds-container" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem', maxWidth: '580px', margin: '0 auto', lineHeight: 1.7 }}>
              ...
              {featured && (
                <Link href={`/blog/${featured.slug}`} style={{ textDecoration: 'none' }}>
                  <article className="blog-card blog-card-featured">
                    ...
                    <h2 style={{ fontSize: 'clamp(1.4rem, 3.5vw, 2rem)', fontWeight: 900, color: 'var(--text-strong)', lineHeight: 1.25, marginBottom: '16px', maxWidth: '720px' }}>
                      {featured.title}
                    </h2>
                    <p style={{ color: 'var(--text-muted)', lineHeight: 1.75, fontSize: '1rem', marginBottom: '28px', maxWidth: '700px' }}>
                      {featured.excerpt}
                    </p>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
                      <div style={{ display: 'flex', gap: '20px', color: 'var(--text-dim)', fontSize: '0.85rem' }}>
                  ...
                  {rest.map(post => (
                    <Link key={post.slug} href={`/blog/${post.slug}`} style={{ textDecoration: 'none' }}>
                      <article className="blog-card blog-card-grid">
                        ...
                        <h2 style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--text-strong)', lineHeight: 1.35, marginBottom: '10px', flexGrow: 1 }}>
                          {post.title}
                        </h2>
                        <p className="blog-excerpt-clamp" style={{ color: 'var(--text-muted)', fontSize: '0.875rem', lineHeight: 1.65, marginBottom: '20px' }}>
                          {post.excerpt}
                        </p>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto' }}>
                          <div style={{ display: 'flex', gap: '12px', color: 'var(--text-dim)', fontSize: '0.78rem' }}>
                          ...
                          <ArrowRight size={16} style={{ color: 'var(--accent-2)' }} />
                        </div>
                      </article>
                    </Link>
                  ))}
