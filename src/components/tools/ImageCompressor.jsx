...
export default function ImageCompressor() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
      ...
      {/* Formato */}
      <div>
        <label style={{ display: 'block', color: 'var(--text)', fontSize: '0.88rem', fontWeight: 600, marginBottom: '10px' }}>Formato de salida</label>
        <div style={{ display: 'flex', gap: '10px' }}>
          {FORMATS.map((f) => (
            <button key={f.key} onClick={() => setFormat(f.key)} style={{ flex: 1, padding: '12px', borderRadius: '10px', cursor: 'pointer', fontWeight: 700, fontSize: '0.9rem', background: format === f.key ? 'linear-gradient(135deg, #0891b2, #7c3aed)' : 'var(--bg-card-alt)', color: format === f.key ? 'var(--text-strong)' : 'var(--text-muted)', border: `1px solid ${format === f.key ? 'transparent' : 'var(--border)'}` }}>
              {f.label}
            </button>
          ))}
        </div>
