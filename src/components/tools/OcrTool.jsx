...
export default function OcrTool() {
  ...
  if (!file) {
    return (
      <label
        ...
        style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '14px', padding: '48px 20px', border: '2px dashed rgba(8,145,178,0.4)', borderRadius: '16px', cursor: 'pointer', background: 'var(--bg-card-alt)', textAlign: 'center' }}
      >
        ...
        <div>
          <p style={{ color: 'var(--text-strong)', fontWeight: 700, fontSize: '1rem', marginBottom: '4px' }}>Toca para elegir una imagen o PDF</p>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>foto de un documento, captura o PDF escaneado</p>
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ position: 'relative', borderRadius: '14px', overflow: 'hidden', background: 'var(--bg-card-alt)', border: '1px solid var(--border)' }}>
        {preview ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '18px' }}>
            ...
            <span style={{ color: 'var(--text)', fontSize: '0.9rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{file.name}</span>
          </div>
        )}
        <button onClick={reset} aria-label="Quitar" style={{ position: 'absolute', top: '10px', right: '10px', background: 'var(--bg-card-alt)', border: '1px solid var(--border)', borderRadius: '8px', padding: '6px', cursor: 'pointer', color: 'var(--accent-4)', display: 'flex' }}>
          <X size={16} />
        </button>
      </div>
      ...
      {text && (
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <label style={{ color: 'var(--text)', fontSize: '0.88rem', fontWeight: 600 }}>Texto extraído</label>
          ...
          <textarea
            ...
            rows={10}
            style={{ width: '100%', boxSizing: 'border-box', background: 'var(--bg-card-alt)', border: '1px solid var(--border)', borderRadius: '10px', padding: '14px', color: 'var(--text-strong)', fontSize: '0.9rem', lineHeight: 1.6, outline: 'none', resize: 'vertical', fontFamily: 'inherit' }}
          />
