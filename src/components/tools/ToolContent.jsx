    ...
    <div style={{ maxWidth: '760px', margin: '64px auto 0' }}>
      ...
      {howItWorks.length > 0 && (
        <section style={{ marginBottom: '56px' }}>
          <h2 style={{ fontSize: 'clamp(1.4rem, 3vw, 1.9rem)', fontWeight: 800, color: 'var(--text-strong)', textAlign: 'center', marginBottom: '32px' }}>
            ¿Cómo funciona?
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
            {howItWorks.map((step, i) => (
              ...
              >
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: 1.6, margin: 0 }}>{step}</p>
              </div>
            ))}
          </div>
        </section>
              ...
              >
                <summary
                  style={{
                    ...
                    padding: '18px 20px', cursor: 'pointer', color: 'var(--text-strong)', fontWeight: 600, fontSize: '0.95rem', listStyle: 'none',
                  }}
                >
                  {f.q}
                  <ChevronDown size={18} className="tool-faq-chevron" style={{ color: accent, flexShrink: 0 }} />
                </summary>
                <p style={{ padding: '0 20px 20px', color: 'var(--text-muted)', fontSize: '0.9rem', lineHeight: 1.7, margin: 0 }}>{f.a}</p>
              </details>
            ))}
          </div>
        </section>
